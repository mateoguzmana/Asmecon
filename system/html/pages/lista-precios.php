...
function retornavalores(descrip, plano, valr)
{ 
   window.opener.document.form1.descri<?=$Cont?>.value=descrip; 
   window.opener.document.form1.plano<?=$Cont?>.value=plano; 
   window.opener.document.form1.vlr<?=$Cont?>.value=valr;
...
{
?>
function retornavalores(descrip, valr)
{ 
   window.opener.document.form1.descri<?=$Cont?>.value=descrip; 
   window.opener.document.form1.vlr<?=$Cont?>.value=valr;
...
<body>
<form name="forma" method="get" action="lista-precios.php">
  <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td height="100" align="right" valign="top"><table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td width="30%" height="35" valign="top" bgcolor="#003366" class="listado2">&nbsp;<span class="listado1">&nbsp; </span>NOMBRE / RAZON SOCIAL<br>
              <span class="listado3">
              &nbsp;
              <input name="Marca" type="text" class="TextField" id="Marca" style="width:99%">
            </span></td>
            <td width="30%" valign="bottom" bgcolor="#003366" class="listado2"><span class="listado3">
              ...
              <input name="Recoge" type="hidden" id="Recoge" value="<?=$Recoge?>">
            </span></td>
            <td width="30%" valign="bottom" bgcolor="#003366" class="listado2">&nbsp;</td>
	...
	if ($Marca1 <> '')
	{
	$paging->agregarConsulta("SELECT* FROM Listrapreciositem where Nombre Like '%$Marca1%' and Muestra = 0 and Activo = 0 order by Nombre, Idcat Asc"); 
	}
	else
	{
	$paging->agregarConsulta("SELECT* FROM Listrapreciositem where Muestra = 0 and Activo = 0 order by Nombre, Idcat Asc"); 
	}
...
$Valor			=number_format($Valor,0,'','.');
			...
			$queryT			="SELECT* FROM Listaprecios where Id = '$Idcat'" ;
			$resultT		=mysql_query($queryT, $id);
			
			While($rowT		=mysql_fetch_array($resultT))
			{
			$NombreT		=strtoupper($rowT["Nombre"]);
			}
...
{
?>
        <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0"  onClick="retornavalores('<?=$Nombre?>', '<?=$NombreT?>', '<?=$Valor?>')">
<?
}
elseif($Recoge == 2)
{
?>
        <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0"  onClick="retornavalores('<?=$Nombre?>', '<?=$Valor?>')">
<?
}
?>
