						...
						<div class="row">
							...
							$MosPerzB			=$rowPerz1["Del"];
								...
								$MosPerz2	=	1;
                                                ...
                                                $queryUSERS			="SELECT* FROM Listrapreciositem where Muestra = 0 GROUP BY Nombre Order by Nombre";
                                                $resultUSERS		=mysql_query($queryUSERS, $id);
                                                
                                                While($rowUSERS		=mysql_fetch_array($resultUSERS))
                                                {
                                                $NombreUSERS		=$rowUSERS["Nombre"];
                                                $IdservicioUSERS	=$rowUSERS["Idcat"];
												$IdUSERS			=$rowUSERS["Id"];
												...
												<tr class="gradeA">
													...
													<td>
                                                    <?
			$queryD			="SELECT* FROM Listrapreciositem where Nombre = '$NombreUSERS' and Muestra = 0 and Activo = 0 order by Nombre" ;
			$resultD		=mysql_query($queryD, $id);
			
			While($rowD		=mysql_fetch_array($resultD))
			{
			$Idservicio		=$rowD["Idcat"];
			

				$queryS			="SELECT* FROM Listaprecios where Id = '$Idservicio' and Muestra = 0 and Activo = 0 order by Nombre" ;
...
?>
													...
													<td class="text-right">
														...
														{
														?>                                                    
														<input name="Procesar" type="button" class="btn btn-primary" value="MODIFICAR"  onClick="location.href='act-asigna-2.php?Id=<?=$IdUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'"> 
                                                        <?
														}
														?> 
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <button type="button" class="btn btn-success" onclick="location.href='eli-asigna.php?Id=<?=$IdUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'">Confirmar</button>
                                                            </div>
                                                        </div><!-- /.modal-content -->
                                                    </div><!-- /.modal-dialog -->
                                                </div><!-- /.modal -->
