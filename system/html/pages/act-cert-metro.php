						...
						<div class="row">
							...
							$MosPerzB			=$rowPerz1["Del"];
								...
								$MosPerz2	=	1;
                                                ...
                                                $queryUSERS			="SELECT* FROM CertificadoMetrologico where Muestra = 0 Order by Id";
                                                $resultUSERS		=mysql_query($queryUSERS, $id);
                                                
                                                while($rowUSERS		=mysql_fetch_array($resultUSERS))
                                                {
												$IdUSERS			=$rowUSERS["Id"];
												$Cliente 			=$rowUSERS["Cliente"];
												$Proveedor 			=$rowUSERS["Proveedor"];
												
												$queryPP			="SELECT * FROM Clientes where Id='$Cliente'";
                                                $resultPP			=mysql_query($queryPP, $id);
                                                
                                                while($rowPP		=mysql_fetch_array($resultPP))
                                                {
                                                $NombreCliente 		=$rowPP["Nombre"];
                                                }
												...
												<tr class="gradeA">
													...
													<td class="text-right">
														...
														{
														?>                                                    
														<input name="Procesar" type="button" class="btn btn-primary" value="MODIFICAR"  onClick="location.href='act-cert-metro-2.php?Id=<?=$IdUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'"> 
                                                        <?
														}
														?> 
													...
													<input name="Eliminar" type="button" class="btn btn-danger" value=" ELIMINAR " data-toggle="modal" data-target="#textModal<?=$IdUSERS?>" data-placement="top">
														...
														?> 
                                                    	<input name="Imprimir" type="button" class="btn btn-support5" value="IMPRIMIR"  onclick="window.open('print-cert-metro.php?Id=<?=$IdUSERS?>');"> 
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                ...
                                                                <h4 class="modal-title" id="textModalLabel">Eliminar certificado metrológico Nro. <?=$IdUSERS?></h4>
                                                            </div>
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <button type="button" class="btn btn-success" onclick="location.href='eli-cert-metro.php?Id=<?=$IdUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'">Confirmar</button>
                                                            </div>
                                                        </div><!-- /.modal-content -->
                                                    </div><!-- /.modal-dialog -->
                                                </div><!-- /.modal -->
