						...
						<div class="row">
							...
							$MosPerzB			=$rowPerz1["Del"];
								...
								$MosPerz2	=	1;
                                                ...
                                                $queryUSERS			="SELECT* FROM Documentacion where Muestra = 0 GROUP BY Nombre Order by Nombre";
                                                $resultUSERS		=mysql_query($queryUSERS, $id);
                                                
                                                While($rowUSERS		=mysql_fetch_array($resultUSERS))
                                                {
                                                $NombreUSERS		=$rowUSERS["Nombre"];
                                                $IdservicioUSERS	=$rowUSERS["Idservicio"];
												$IdUSERS			=$rowUSERS["Id"];
												...
												<tr class="gradeA">
													...
													<td>
                                                    <?
			$queryD			="SELECT* FROM Documentacion where Nombre = '$NombreUSERS' and Muestra = 0 and Activo = 0 order by Nombre" ;
			$resultD		=mysql_query($queryD, $id);
			
			While($rowD		=mysql_fetch_array($resultD))
			{
			$Idservicio		=$rowD["Idservicio"];

				$queryS			="SELECT* FROM Servicios where Id = '$Idservicio' and Muestra = 0 and Activo = 0 order by Nombre" ;
				$resultS		=mysql_query($queryS, $id);
				
				While($rowS		=mysql_fetch_array($resultS))
				{
				$Servicio		=$rowS["Nombre"];


?>                                                    <button type="button" class="btn btn-primary btn-xs"><?=$Servicio?></button>
<?
				}
			}
?>
													...
													<td class="text-right">
														...
														{
														?>                                                    
														<input name="Procesar" type="button" class="btn btn-primary" value="MODIFICAR"  onClick="location.href='act-documento-2.php?Id=<?=$IdUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'"> 
                                                        <?
														}
														?> 
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <button type="button" class="btn btn-success" onclick="location.href='eli-documento.php?Nombre=<?=$NombreUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'">Confirmar</button>
                                                            </div>
                                                        </div><!-- /.modal-content -->
                                                    </div><!-- /.modal-dialog -->
                                                </div><!-- /.modal -->
