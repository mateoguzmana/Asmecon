...
?>
		...
		$Factura		=$row["Factura"];
			...
			$Idserv			=$roY["Idserv"];
																		...
																		if($TOTAL > 0 && $Idserv == 2)
																		{
																			$queryTIT		="SELECT* FROM Fdimensional where Idcotiz = '$Idcotiz' and Idorden = '$Id'";
																			$resultTIT		=mysql_query($queryTIT, $id);
												...
												<tr class="gradeA">
													...
													<td class="text-right">
														...
														{
														?>                                                    
														<input name="Procesar" type="button" class="btn btn-primary" value="MODIFICAR"  onClick="location.href='act-formato-d-2.php?Idord=<?=$Id?>&Idccotiz=<?=$Idcotiz?>&Id=<?=$Idf?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'"> 
                                                        <input name="Procesar" type="button" class="btn btn-danger" value="ELIMINAR"  data-toggle="modal" data-target="#textModal<?=$Idf?>" data-placement="top"> 
														<input name="Procesar" type="button" class="btn btn-support5" value="IMPRIMIR" onclick="window.open('print-formato-d.php?Idord=<?=$Id?>&Idccotiz=<?=$Idcotiz?>&Id=<?=$Idf?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>');window.open('excel-formato-d.php?Id=<?=$Idf?>');"> 
														<?
														}
														?> 
                                                ...
                                                <div class="modal fade" id="textModal<?=$Idf?>" tabindex="-1" role="dialog" aria-labelledby="textModalLabel" aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                                                                <h4 class="modal-title" id="textModalLabel"><?=$Nombre?> - <?=$Idcotiz?></h4>
                                                            </div>
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <button type="button" class="btn btn-success" onclick="location.href='eli-formato.php?Id=<?=$Idf?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'">Confirmar</button>
                                                            </div>
                                                        </div><!-- /.modal-content -->
                                                    </div><!-- /.modal-dialog -->
                                                </div><!-- /.modal -->
