...
include("../../includes/conexion.php");
                    ...
                    $queryped			="SELECT * FROM OpcMediciones WHERE Id = '$Idt'";
					...
					$SubCategoria		=$rowped["SubCategoria"];
					$Inicial 			=$rowped["Inicial"];
					}
					
					$queryKK 			="SELECT * FROM SubMediciones WHERE Id='$SubCategoria'";
									...
									<div class="tab-content">
										...
										<div class="tab-pane active" id="editDetails">
											<div class="box-body style-white">
												<form class="form-horizontal form-validate" role="form" method="post" action="act-opcmedidas-3.php" novalidate>
																...
																<div class="col-sm-12">
																	...
																	<input name="Linea" id="Linea" type="hidden" value="<?=$Linea?>">
																	  <select name="jumpMenu" id="jumpMenu" class="form-control" onChange="MM_jumpMenu('parent',this,0)" required>
																		...
																		{
																		?>
                                                                 		<option value="act-opcmedidas-2.php?Id=<?=$Idt?>&Linea=<?=$Linea?>&Idmenu=<?=$Idmenux?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>" selected><?=$NombreQQ?></option>
                                                                        <?
																		}
																		else{
																		?>
                                                                        ...
                                                                        ?>
                                                                        <option value="act-opcmedidas-2.php?Id=<?=$Idt?>&Linea=<?=$Id88?>&Categoria=x&Idmenu=<?=$Idmenux?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>"><?=$Nombre88?></option>
																		<?
																		}
																		?>
                                                                </select>
															...
															<div class="form-group">
																<div class="col-sm-12">
																	...
																	<input name="Categoria" id="Categoria" type="hidden" value="<?=$Categoria?>">
																	  <select name="jumpMenu" id="jumpMenu" class="form-control" onChange="MM_jumpMenu('parent',this,0)" required>
            															<?if(!empty($Categoria)){?>
                                                                        <option value="act-opcmedidas-2.php?Id=<?=$Idt?>&Linea=<?=$Linea?>&Categoria=<?=$Categoria?>&Idmenu=<?=$Idmenux?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>" selected><?=$NombreFF?></option>
                                                                        <?}else{?>
                                                                        <option value="" selected>Seleccione una Categoria</option>
																		<?
																		}
                                                                        $query33		="SELECT * FROM CatMediciones where Id <> '$Categoria' and Muestra = 0 AND Linea='$Linea'  order by Nombre";
                                                                        ...
                                                                        ?>
                                                                        <option value="act-opcmedidas-2.php?Id=<?=$Idt?>&Linea=<?=$Linea?>&Categoria=<?=$Id33?>&Idmenu=<?=$Idmenux?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>"><?=$Nombre33?></option>
																		<?
																		}
																		?>
                                                                </select>
																...
																<div class="col-sm-12">
																	<label for="SubCategoria" class="control-label">Sub Categoria</label>
																	 <select name="SubCategoria" id="SubCategoria" class="form-control select2-list" data-placeholder="Seleccione Categoria" required>
            															<?if(empty($Cat) && $Categoria!="x"){?>
                                                                        <option value="<?=$IdKK?>" selected><?=$NombreKK?></option>
                                                                        <?}else{?>
                                                                        <option value="" selected>Selecciona una Sub-categoria</option>
																		<?}
															...
															<div class="form-group">
                                                              <div class="col-sm-12">
																<label for="Inicial" class="control-label">Referencia del instrumento</label>
																	<input type="text" name="Inicial" id="Inicial" class="form-control" placeholder="Ingrese un código" onChange="javascript:this.value=this.value.toUpperCase();" value="<?=$Inicial?>" required>
																</div>
															</div>
														</div>
													</div>
